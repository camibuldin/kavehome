            ...
            <div key={item.productSku} className={styles.productContainer}>
              <div className={styles.productDiv}>
                ...
                >
                  ...
                  <svg onClick={() => removeFromFavs(item.productSku)} xmlns="http://www.w3.org/2000/svg" width="23" height="21" viewBox="0 0 23 21" fill="none">
                    <path
                      fill-rule="evenodd"
                      clip-rule="evenodd"
        ...
        <div className={styles.paginationContainer}>
          {totalPages > 1 && (
            <div className={styles.paginationDiv}>
              {currentPage > 4 && (
                <button onClick={() => setCurrentPage(currentPage - 1)} className={styles.backBtn}>
                  <Image src="/flecha-correcta.png" alt="Previous" width={10} height={10} />
                </button>
              )}
              {Array.from({ length: endIndex - startIndex }, (_, index) => (
                <button
                  className={`${styles.paginationBtn} ${currentPage === index + startIndex + 1 ? styles.selectedPage : ""}`}
              ...
              <button onClick={() => setCurrentPage(currentPage + 1)} disabled={currentPage === totalPages} className={styles.nextBtn}>
                <Image src="/flecha-correcta.png" alt="Next" width={10} height={10} />
              </button>
            </div>
          )}
        </div>
